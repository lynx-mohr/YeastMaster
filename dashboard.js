...
// ==========================================
const shareTexts = {
    en: { btn: "Share", title: "Share this fermentation", desc: "Anyone with this link can view this fermentation (read-only). No account needed.", copy: "Copy link", copied: "Copied!", stop: "Stop sharing", close: "Close", banner: "Viewing a shared fermentation", failed: "Could not create the link. Try again." },
    sv: { btn: "Dela", title: "Dela den här jäsningen", desc: "Alla med den här länken kan se jäsningen (skrivskyddat). Inget konto behövs.", copy: "Kopiera länk", copied: "Kopierad!", stop: "Sluta dela", close: "Stäng", banner: "Du tittar på en delad jäsning", failed: "Kunde inte skapa länken. Försök igen." },
    de: { btn: "Teilen", title: "Diese Gärung teilen", desc: "Jeder mit diesem Link kann diese Gärung sehen (schreibgeschützt). Kein Konto nötig.", copy: "Link kopieren", copied: "Kopiert!", stop: "Teilen beenden", close: "Schließen", banner: "Du siehst eine geteilte Gärung", failed: "Link konnte nicht erstellt werden. Versuch es erneut." },
    fr: { btn: "Partager", title: "Partager cette fermentation", desc: "Toute personne ayant ce lien peut voir cette fermentation (lecture seule). Aucun compte requis.", copy: "Copier le lien", copied: "Copié !", stop: "Arrêter le partage", close: "Fermer", banner: "Tu regardes une fermentation partagée", failed: "Impossible de créer le lien. Réessaie." },
    es: { btn: "Compartir", title: "Comparte esta fermentación", desc: "Cualquiera con este enlace puede ver esta fermentación (solo lectura). No hace falta cuenta.", copy: "Copiar enlace", copied: "¡Copiado!", stop: "Dejar de compartir", close: "Cerrar", banner: "Estás viendo una fermentación compartida", failed: "No se pudo crear el enlace. Inténtalo de nuevo." }
};
...
window.openShareModal = async function() {
    ...
    try {
        ...
        if (input) input.value = (res.ok && d.token) ? `${location.origin}/s/${d.token}?lang=${window.currentLang || 'en'}` : t.failed;
    } catch (e) {
