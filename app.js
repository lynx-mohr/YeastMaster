...
// ==========================================
// --- YEAST LIBRARY TOUR (EPIC CROSS-VIEW VERSION) ---
// ==========================================
let currentLibStep = -1;
let libTourSteps = [];
window.isLibraryTourActive = false; 
let isTourAnimating = false; 

// Frys-funktioner (Inget hopp åt höger!)
...
window.startLibraryTour = function() {
    ...
    libTourSteps = [
        ...
        // Steg 3: Hoppa rakt in i The Profiler
        {
            selector: '#lab-chart',
            text: 'Welcome to The Profiler! Here you can visually drag the temperature points to tweak the fermentation curve.',
            action: () => {
                closeYeastModal();
                
                // Vi låser oavsiktligt upp skärmen när vi stänger modalen, frys igen!
                document.body.style.overflow = 'hidden'; 
        ...
        // Steg 4: Dry Hops & Racking i Labbet
        {
            selector: '#btn-add-hops', // Byt ut denna selector om dina knappar heter något annat
            text: 'Set your alarms for Dry hops and racking! They will show up on your timeline.',
            alignLeft: true,
            action: () => {
                // Frivilligt: Om du har funktioner för att visa linjerna i grafen kan vi anropa dem här
                // toggleDryHopLine();
            }
        },
        // Steg 5: Tillbaka till Library och visa stjärnan
        {
            selector: '#tour-fake-custom-card',
            text: 'When saved, your modded hardware profile lands at the bottom of the library with a ★ star, ready to be synced!',
            action: () => {
                if (typeof showView === 'function') showView('library');
                
                // Trolleri: Skapa ett temporärt fejk-kort med stjärna
                ...
                if (!fakeCard) {
                    ...
                    fakeCard.className = 'yeast-card custom-profile'; // Använd dina egna CSS-klasser
                    fakeCard.style.borderColor = 'var(--accent-color)';
                    fakeCard.style.marginTop = '20px';
                    fakeCard.innerHTML = '<h3 style="color: var(--accent-color); text-align: center; margin: 15px 0;">★ My Modded US-05</h3>';
                    
                    const grid = document.getElementById('yeast-grid');
                    if (grid) grid.appendChild(fakeCard);
                }
                
                // Scrolla mjukt ner till det nya fejk-kortet
                setTimeout(() => {
...
window.nextLibraryTourStep = function(e) {
    ...
    if (currentLibStep >= libTourSteps.length) {
        ...
        // --- STÄDA BORT FEJK-KORTET ---
        const fakeCard = document.getElementById('tour-fake-custom-card');
        if (fakeCard) fakeCard.remove();

        // Se till att vi stänger modalen om den råkat vara öppen
        if (typeof closeYeastModal === 'function') closeYeastModal();
        
        // Se till att vi är i library-vyn när touren slutar
        if (typeof showView === 'function') showView('library');
